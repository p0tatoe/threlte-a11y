<script lang="ts">
	...
	import announceStore from './announceStore.ts';
	...
	const pressed = writable(startPressed);
	...
	function activate() {
		pressed.update((v) => {
			if (v) {
				if (deactivationMsg) announceStore.a11yScreenReader(deactivationMsg);
			} else {
				if (activationMsg) announceStore.a11yScreenReader(activationMsg);
			}
			return !v;
		});
		actionCall?.();
		dispatch('click');
		dispatch('toggle', { pressed: !$pressed }); // $pressed is old value here? No, store update runs immediately? Wait.
		// In Svelte store update callback, we return the new value.
		// But to dispatch the *new* value, we might want to do it outside or check logic.
		// Actually, let's trust the reactive $pressed in the template, but for dispatching 'toggle', maybe we should just let the parent subscribe to the store or actionCall.
		// The React code calls actionCall().
	}

	// Cursor
	$: if (domElement) {
		domElement.style.cursor = $hovered && !disabled ? 'pointer' : 'default';
	}
...
<T.Group
	...
	onclick={(e: any) => {
		e.stopPropagation();
		if (disabled || (dragThreshold && e.delta > dragThreshold)) return;
		activate();
	}}
>
	<slot />
